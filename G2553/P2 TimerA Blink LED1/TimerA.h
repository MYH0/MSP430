...
void TA1_Init(char ta0_clk0, int ta0_div0, int ta0_method0, int time_long)
{
	TA1CTL = 0;				//清除以前的设置
	TA1CCTL0 |= CCIE;		//使能“捕获/比较”寄存器，以允许存储阈值
	TA1CTL |= TACLR;		//计数器清0
	TA1_CLK(ta0_clk0);		//设定时钟源
	TA1_DIV(ta0_div0);        //设定时钟分频(默认是不用的)	
	TA1_Method(ta0_method0);  //设定计数模式
